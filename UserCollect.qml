...
Rectangle {
    function show(){
        ...
        userCollectAnimation.from=newsWindow.x;
        userCollectAnimation.to=0;
        userCollectAnimation.restart();
        newsListAnimation.restart();
        collectNewsList.model=TL.getCollectNews()
    }
    function back(){
        newsListAnimation.from=-newsList.width/3;
        newsListAnimation.to = 0;
        userCollectAnimation.from=0;
        userCollectAnimation.to=newsWindow.width;
        userCollectAnimation.restart();
        newsListAnimation.restart();
    }
